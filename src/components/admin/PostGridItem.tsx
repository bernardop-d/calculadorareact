...
import { Eye, EyeOff, ImageIcon, Pencil, Trash2, CheckCircle2 } from "lucide-react";
...
export default function PostGridItem({ post, onEdit, onDelete, selecting, selected, onToggleSelect }: Props) {
  const tier = post.contentTier ?? "PREMIUM";
  const media = post.media[0];

  function handleClick() {
    if (selecting && onToggleSelect) onToggleSelect(post.id);
  }

  return (
    <div
      className={`relative aspect-square group overflow-hidden bg-zinc-900 cursor-pointer ${selecting && selected ? "ring-2 ring-[#F5C400] ring-inset" : ""}`}
      onClick={handleClick}
    >
      {/* Thumbnail */}
      {media ? (
      ...
      {/* Modo seleção */}
      {selecting ? (
        <div className={`absolute inset-0 transition-colors ${selected ? "bg-[#F5C400]/20" : "bg-black/10 hover:bg-black/30"}`}>
          <div className="absolute top-2 right-2">
            {selected
              ? <CheckCircle2 size={22} className="text-[#F5C400] drop-shadow" fill="currentColor" />
              : <div className="w-5 h-5 rounded-full border-2 border-white/60 bg-black/40" />}
          </div>
          {!post.published && (
